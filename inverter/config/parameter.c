...
typedef struct
{
    ...
    AccessType    access;
    const char    *label;
    uint8_t       unit[2]; // Added 2-byte unit array
} Parameter;

static const Parameter param_table[] = {
    // --- ADC Config (0x1000) ---
    {0x1000, &app_config.adc.steps,                TYPE_UINT16,  ACCESS_RW, "ADC_STEPS",     {' ',  ' ' }},
    {0x1001, &app_config.adc.supply,               TYPE_FLOAT32, ACCESS_RW, "ADC_SUPPLY",    {'m',  'V' }},
    ...
    {0x1102, &app_config.current.current_sense_factor, TYPE_FLOAT32, ACCESS_RW, "CUR_SENSE_FAC", {'A',  '/' }}, // e.g. A/V or A/LSB

    // --- PWM Config (0x1200) ---
    {0x1200, &app_config.pwm.frequency,            TYPE_UINT32,  ACCESS_RW, "PWM_FREQ",      {'H',  'z' }},

    // --- FOC Config (0x1300) ---
    {0x1300, &app_config.foc.motor_polepairs,      TYPE_UINT8,   ACCESS_RW, "MOTOR_POLES",   {' ',  ' ' }},
    {0x1301, &app_config.foc.speedRefHz,           TYPE_FLOAT32, ACCESS_RW, "SPEED_REF",     {'H',  'z' }},
    {0x1302, &app_config.foc.voltageRef,           TYPE_FLOAT32, ACCESS_RW, "VOLT_REF",      {'V',  ' ' }},
    {0x1303, &app_config.foc.calibration_ticks,    TYPE_UINT64,  ACCESS_RW, "CALIB_TICKS",   {' ',  ' ' }},
    {0x1304, &app_config.foc.resolver_offset,      TYPE_FLOAT32, ACCESS_RW, "RES_OFFSET",    {'d',  'g' }},
    {0x1305, &app_config.foc.id_ref,               TYPE_FLOAT32, ACCESS_RW, "ID_REF",        {'A',  ' ' }},
    {0x1306, &app_config.foc.iq_ref,               TYPE_FLOAT32, ACCESS_RW, "IQ_REF",        {'A',  ' ' }},
    ...
    {0x1402, &app_config.foc.pi_config_id.outMax,  TYPE_FLOAT32, ACCESS_RW, "ID_PI_MAX",     {'V',  ' ' }},
    {0x1403, &app_config.foc.pi_config_id.outMin,  TYPE_FLOAT32, ACCESS_RW, "ID_PI_MIN",     {'V',  ' ' }},
    ...
    {0x1502, &app_config.foc.pi_config_iq.outMax,  TYPE_FLOAT32, ACCESS_RW, "IQ_PI_MAX",     {'V',  ' ' }},
    {0x1503, &app_config.foc.pi_config_iq.outMin,  TYPE_FLOAT32, ACCESS_RW, "IQ_PI_MIN",     {'V',  ' ' }},
    ...
    {0x2100, &app_state.foc.i_ab.alpha,            TYPE_FLOAT32, ACCESS_R,  "I_ALPHA",       {'A',  ' ' }},
    {0x2101, &app_state.foc.i_ab.beta,             TYPE_FLOAT32, ACCESS_R,  "I_BETA",        {'A',  ' ' }},
    {0x2102, &app_state.foc.v_ab.alpha,            TYPE_FLOAT32, ACCESS_R,  "V_ALPHA",       {'V',  ' ' }},
    {0x2103, &app_state.foc.v_ab.beta,             TYPE_FLOAT32, ACCESS_R,  "V_BETA",        {'V',  ' ' }},
    {0x2104, &app_state.foc.i_dq.d,                TYPE_FLOAT32, ACCESS_R,  "I_D",           {'A',  ' ' }},
    {0x2105, &app_state.foc.i_dq.q,                TYPE_FLOAT32, ACCESS_R,  "I_Q",           {'A',  ' ' }},
    {0x2106, &app_state.foc.v_dq.d,                TYPE_FLOAT32, ACCESS_R,  "V_D",           {'V',  ' ' }},
    {0x2107, &app_state.foc.v_dq.q,                TYPE_FLOAT32, ACCESS_R,  "V_Q",           {'V',  ' ' }},
    ...
    {0x2400, &app_state.resolver.prev_elec_angle,  TYPE_FLOAT32, ACCESS_R,  "PREV_ANGLE",    {'d',  'g' }},
...
bool getParameterInfo(uint16_t index, uint16_t *address, uint8_t *type, uint8_t *access, const char **label, uint8_t *unit) {
    if (index < PARAM_TABLE_SIZE) {
        ...
        if (access) *access = (param_table[index].access == ACCESS_RW) ? 1 : 0;
        if (label) *label = param_table[index].label;
        if (unit) {
            unit[0] = param_table[index].unit[0];
            unit[1] = param_table[index].unit[1];
        }
        return true;
    }
    return false;
